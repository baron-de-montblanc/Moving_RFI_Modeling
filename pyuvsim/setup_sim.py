...
def plot_trajectory(
    ...
    ax.scatter(
        ra_pix, dec_pix, s=1, label=f"Satellite tracks (duration {round(duration)}s)"
    )
    ax.scatter(
        zra_pix,
        zdec_pix,
        ...
        s=100,
        color="red",
        label="Zenith from MWA on start time",
    )
    ...
    for i, t in enumerate(tqdm(time_array)):
        ...
        df = pd.DataFrame(
            ...
            data=[
                [
                    f"src{pdx}",
                    float(ra_array[i].value),
                    float(dec_array[i].value),
                    src_flux,
                    src_freq,
                ]
            ],
...
def _get_mwa_zenith(date):
    ...
    return SkyCoord(
        alt=90 * u.deg, az=0 * u.deg, frame=AltAz(obstime=date, location=mwa)
    ).icrs
...
def _deg_to_pix(wcs, xdeg, ydeg):
    ...
    pix_coords = wcs.wcs_world2pix(
        np.column_stack((coord.ra.degree, coord.dec.degree)), 0
    )
...
if __name__ == "__main__":
    ...
    time_array = (
        start_time.jd + np.arange(num_samples) * time_res / 86400.0
    )  # seconds to days
    ...
    plot_trajectory(
        traj_ra, traj_dec, label, start_time, duration, savepath=plot_savename
    )
